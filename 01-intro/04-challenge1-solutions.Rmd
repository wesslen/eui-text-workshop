...
2. Find the most common hashtags (function `getCommonHashtags`); make sure to use the Tweet text (body) as the input. What are the top 25 hashtags?

 * Repeat the same exercise with the handle function (`getCommonHandles`). 

 * Use the `kwic` function to find other panther words.
...
head(kwic(twcorpus, "panthers", window=5),10)
```

3. Choose multiple keywords (could include hashtags, handles) that you think will best identify Carolina Panther Tweets. 
